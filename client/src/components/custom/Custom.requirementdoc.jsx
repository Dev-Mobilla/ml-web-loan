...
import {EmployedRequirementComponent, SelfEmployedRequirementComponent} from '../index';
import {SliceIMageName} from '../../utils/SliceImageName';
import {useModal} from '../../utils/modalContext';
import {MakeRed} from '../../utils/DataFunctions';
import file from "../../assets/images/file.png";

const CustomRequirementComponent = ({ 
  availableDocuments, orDoc
 }) => {
  ...
  return (
    <div className="requirement-group">
      {
        availableDocuments?.map((document, key) => {
          return (
            <div
              className="requirement"
              onClick={() => openModal(document.name, file)}
              key={key}
            >
              <img
              ...
              <div className="requirement-text">
                <div className="requirement-title">{document.name}</div>
                <div className="re-file">
                  <div className="re-close">[x]</div>
                  <div className={`requirement-file ${MakeRed(document.value)}`}>
                    {document.value == null ? (
                      "none"
                    ) : document.value.imageName === "" ? (
                      <span style={{ color: "red" }}>none</span>
                    ) : (
                      SliceIMageName(document.value.imageName)
                    )}
                  </div>
                </div>
              </div>
